...
type socksDialer struct {
	...
	// LocalDNS controls DNS resolution for the target address.
	// When false (default), hostnames are forwarded to the proxy for resolution
	// (SOCKS5h semantics). When true, hostnames are resolved locally before
	// the CONNECT request is sent (classic SOCKS5 semantics).
	LocalDNS bool
