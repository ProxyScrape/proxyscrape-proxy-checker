    ...
    return (
        <>
            {guestMode && (
                <GuestModeBanner
                    feature="IP settings"
                    description="Configuring a custom judge URL or IP override lets you accurately detect anonymity levels. These settings require the desktop app."
                />
            )}
            <Box sx={{ bgcolor: 'background.paper', borderRadius: 3, p: 2.5, mb: 2 }}>
                ...
                <HelpTip title="URL that returns your public IP address in plain text" placement="bottom">
                    <TextField
                        ...
                        name="lookupUrl"
                        placeholder="Url"
                        onChange={guestMode ? () => {} : changeOption}
                        value={lookupUrl}
                        disabled={guestMode}
                    />
                </HelpTip>
                ...
                <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                    <HelpTip title={current ? 'Manual override active — auto-detection is skipped. Clear this field to resume auto-detection.' : 'Auto-detected before each check using the lookup URL above.'} placement="bottom">
                        <TextField
                            ...
                            onChange={guestMode ? () => {} : changeOption}
                            value={guestMode ? '' : current}
                            disabled={guestMode}
                        />
                    </HelpTip>
                    <Button variant="contained" size="small" onClick={guestMode ? undefined : IpLookup} disabled={guestMode} sx={{ whiteSpace: 'nowrap' }}>
                        Check
                    </Button>
                </Stack>
            </Box>
