...
const Blacklist = ({ filter, items, changePath, add, remove, setActive, toggleOption, selectPath }) => {
    ...
    return (
        <>
            {guestMode && (
                <GuestModeBanner
                    feature="Blacklist settings"
                    description="Blacklists let you screen results against known bad IPs and CIDR ranges. Adding and managing blacklist sources requires the desktop app."
                />
            )}
            <Box sx={{ bgcolor: 'background.paper', borderRadius: 3, p: 2.5, mb: 2 }}>
                ...
                <Box>
                    {items.map(item => (
                        <BlacklistItem
                            key={item.title}
                            changePath={guestMode ? () => {} : changePath}
                            remove={guestMode ? () => {} : remove}
                            setActive={guestMode ? () => {} : setActive}
                            selectPath={guestMode ? () => {} : selectPath}
                            readOnly={guestMode}
                            {...item}
                        />
                    ))}
